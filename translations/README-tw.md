...
> 正規表達式是一组由字母和符號组成的特殊文體，它可以用來從文章中找出滿足你想要的格式的句子。

一個正規表達式是一種從左到右匹配主體字符串的模式。
“Regular expression” 這個詞比較拗口，我們常使用縮寫的術語為 “regex” 或 “regexp” 。
正則表達式可以從一個基礎字符串中根據一定的匹配模式替換文本中的字符串、驗證表單、提取字符串等等。

想像你正在寫一個應用，然後你想設定一個用戶命名的規則，讓用戶名稱包含字符、數字、下劃線和連字符，以及限製字符的個數，好讓名字看起來沒那麼醜。
我們使用以下正規表達式來驗證一個用戶名：
...
* [5. 標誌](#5-標誌)
	...
	* [5.2 全域搜索（Global search）](#52-全域搜索-global-search)
